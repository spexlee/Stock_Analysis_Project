...
def ReqeustData(obj):
    ...
    for i in range(count):
        day = obj.GetDataValue(0, i)  # 일자 (day)
        cur_pr = obj.GetDataValue(1, i)  # 시가 (cur_pr)
        high_pr = obj.GetDataValue(2, i)  # 고가 (high_pr)
        low_pr = obj.GetDataValue(3, i)  # 저가 (low_pr)
        clo_pr = obj.GetDataValue(4, i)  # 종가 (clo_pr)
        pr_diff = obj.GetDataValue(5, i)  # 전일대비 (pr_diff)
        acc_vol = obj.GetDataValue(6, i)  # 누적거래량 (acc_vol)
        for_stor = obj.GetDataValue(7, i)  # 외인보유 (for_stor)
        for_stor_diff = obj.GetDataValue(8, i)  # 외인보유전일대비 (for_stor_diff)
        for_perc = obj.GetDataValue(9, i)  # 외인비중 (for_perc)
        com_buy_vol = obj.GetDataValue(12, i)  # 기관순매수수량 (com_buy_vol)
        oot_cur_pr = obj.GetDataValue(13, i)  # 시간외단일가시가 (oot_cur_pr)
        oot_high_pr = obj.GetDataValue(14, i)  # 시간외단일가고가 (oot_high_pr)
        oot_low_pr = obj.GetDataValue(15, i)  # 시간외단일가저가 (oot_low_pr)
        oot_clo_pr = obj.GetDataValue(16, i)  # 시간외단일가종가 (oot_clo_pr)
        oot_pr_diff = obj.GetDataValue(18, i)  # 시간외단일가전일대비 (oot_pr_diff)
        oot_vol = obj.GetDataValue(19, i)  # 시간외단일가거래량 (oot_vol)
