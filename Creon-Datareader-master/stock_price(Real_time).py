...
class CpEvent: # 실시간 조회
    ...
    def OnReceived(self):
        ...
        conn = sqlite3.connect("stock(Real_time).db", isolation_level=None)
        c = conn.cursor() # cursor : ? 를 받을 수 있는 것
        ...
        c.execute("CREATE TABLE IF NOT EXISTS " + code +
                  ...
                  ", acc_buy_deal_vol integer,moment_deal_vol integer, time_sec integer, exFlag text )")

        # sql문 실행 - 테이블 생성
        c.execute(
            "INSERT OR IGNORE INTO " + code + " VALUES( ?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
            ((diff,time,cur_price,high_price,low_price,sell_call,buy_call,acc_vol,pred_price,deal_state,acc_sell_deal_vol,acc_buy_deal_vol,moment_deal_vol,time_sec,exFlag)))


        if (exFlag == ord('1')):  # 동시호가 (예상체결)
            print("실시간(예상 체결)", code,name,diff,time,cur_price,high_price,low_price,sell_call,buy_call,acc_vol,pred_price,deal_state,acc_sell_deal_vol
                  ,acc_buy_deal_vol,moment_deal_vol,exFlag)
